...
Manual tax payments use a dedicated structured record rather than a Thought. The record keeps the applicable tax year separate from its submission and status dates, exact amount and currency, payer and authority, confirmation or EFT identity, optional receipt evidence, and actor/time provenance. Settlement, rejection, reversal and explicit correction update the same payment while an append-only status history records the reason. Year totals group by both currency and current status so a rejected or reversed payment is never presented as settled. This bounded capture surface does not calculate tax liability or create a general ledger.
