...
| Sharing the work     | Public code, synthetic examples, simple setup instructions and independent private deployments. MIT is the intended permissive license.                         | Upstream code-grant scope needs confirmation; plugin metadata alone is not treated as conclusive for the whole inherited repository. No hosted service or app-store launch is required. |
