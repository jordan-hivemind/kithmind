...
| P2-39j    | Deferred work, sweeps and diagnostics. `kith.deferred_work`, the daemon command, the launchd job, read-time worker staleness, incidents and watcher resets, one daily durable incident record. 787 lines. P2-39j2 registered the `embedding_fill` handler: every thought write schedules the fill in its own transaction under a per-space dedupe key, and the daemon's drain runs it with no `pg` connection held across the provider call. `card_queue_tick` stays unregistered until `models/records/cardQueue.ts` is ported. | 1    |      14 | e, f, g    |
