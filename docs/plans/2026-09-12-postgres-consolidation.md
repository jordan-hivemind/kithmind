...
8. Re-embed with `kith-reembed`, then run section 4's post-cutover subset.
   Vectors are not migrated but `covered_fingerprint` is, so a migrated target
   claims coverage no vector row backs and the provider fill skips it. Run
   `kith-reembed` for the per-space counts, then `kith-reembed --apply` to put
   those targets back on the owed index and queue the fill the daemon drains.
   It is idempotent: a space with nothing to repair is left alone.
...
| Encrypted dump and restore        | Yes, into a second isolated server, with tamper rejection              | The production age recipient design and restic identity checks against the owner's real deployed repository. P2-39k (2026-09-16) closed the "both schemas" gap: `scripts/db-backup-postgres.mjs` dumps `finance` and `kith` together with writer quiescence, and `scripts/db-restore-proof.mjs --isolated` proves byte-exact restore parity and one sampled cited answer, both against a synthetic fixture. |
...
| `embeddingVectors` row contents                                 | 180 rows today                                                           | Do not migrate vectors. They are derived and content-addressed by `inputHash`. Re-embed after cutover with `kith-reembed --apply`, which is cutover step 8. 180 targets is a few cents and it avoids a float64 to float32 conversion argument entirely. `kith-migrate` still carries `embeddingTargets.coveredFingerprint`, so the step exists to clear the coverage those migrated rows claim.        |
...
| P2-39j    | Deferred work, sweeps and diagnostics. `kith.deferred_work`, the daemon command, the launchd job, read-time worker staleness, incidents and watcher resets, one daily durable incident record. 787 lines. P2-39j2 registered the `embedding_fill` handler and scheduled it from every thought write. A P2-39j follow-up closed the same gap for the other writers that make a target eligible and uncovered: a worker's chunk publication (`touchWorkerPublicationEmbedding`) and a paged build that completes with a target its own fill phase could not match to an existing vector. Each schedules the fill in its own transaction under a per-space dedupe key, and the daemon's drain runs it with no `pg` connection held across the provider call. `card_queue_tick`, and the card write it would carry, stay unregistered until `models/records/cardQueue.ts` is ported. | 1    |      14 | e, f, g    |
...
| P2-39m    | Parity run, independent security review, cutover, teardown and the cost line closed out. **Gate on the flag flip: duplicate detection must be verified end to end on the PostgreSQL surface first.** The capture admission gate landed with the P2-39i4 follow-up, is fail-closed when the classifier is unavailable, and keeps the SKIP rules for credentials and sensitive content. What is not yet proven in a running deployment is the part that depends on the embedding index: a stored capture leaves an eligible, uncovered target, so I9 reports the space's thought index incomplete and the vector leg drops every candidate until a fill runs. Three things stand in for it -- a byte-identical-retry guard, a keyword candidate leg, and a scheduled fill -- and all three now exist: the fill is scheduled from every writer that makes a target eligible and uncovered (`captureThought` and `transitionMemory` for a thought, `touchWorkerPublicationEmbedding` for a worker's chunk publication, and a paged build that completes with an unmatched target; one `embedding_fill` job per space under a dedupe key, inside the write's own transaction) and drained by `kith-deferred-work` through a pool-scoped handler that holds no transaction across the provider call, so the space's vector leg recovers on its own instead of staying `unavailable` from the first capture onwards. Verify all three against real captures before flipping. Steps 1 to 8 and the load half of step 9 are executable as the `workflow_dispatch` workflow `.github/workflows/cutover.yml`, operated from [the cutover runbook](./2026-09-16-cutover-runbook.md). | 2    |      22 | all        |
| **Total** |                                                                                                                                                                                                                                                                                    |      | **402** |            |
