...
| P2-39m    | Parity run, independent security review, cutover, teardown and the cost line closed out. **Blocker: `KITH_POSTGRES_SURFACE` must not flip to `postgres` until narrative capture's model-backed admission gate is ported.** P2-39i4 shipped `capture_thought` without it, which inverts the gate's failure direction from fail-closed to fail-open and drops SKIP for sensitive content including credentials, as well as duplicate detection and supersession. See section 6 row i4 of the web and MCP surface plan and the module comment in `apps/web/src/lib/mcp/writes.ts`. | 2    |      22 | all        |
| **Total** |                                                                                                                                                                                                                                                                                    |      | **402** |            |
