...
The response reports source item, revision, generation, job, and document IDs plus `ready`, `queued`, `needs_review`, or `failed`, the current desired processing epoch, and whether the accepted generation is active. Retrying the same request ID and canonical validated arguments is a no-op that returns the same authorized result. Reusing a request ID with different validated arguments is a conflict. A new request ID for the same source external ID and changed text creates a new immutable revision and activates it only after complete staging. The caller supplies `expectedDesiredProcessingEpoch`, initially zero, then the returned current epoch when replacing a source. Repeating a request uses its original epoch and original arguments. This compare-and-swap prevents a delayed client from overwriting a newer correction. A historical receipt never reactivates an old generation.

`ingest_url` stores a durable fetch request against a stable source item, without inventing a fetched revision or document. Phase 1 does not fetch it and returns `queued` with `workerRequired: true`. Pending fetch requests count as incomplete source coverage and are removed by source forgetting. URL fetching, redirect and private-network defenses, file downloads, OCR, portal automation, and connector credentials belong to Phase 2. Tests must not make network requests.
