...
Card targets depend on the target table from the first capacity PR. The card
embedding PR in section 12 must not land before it. Nothing else in this plan
depends on the capacity work, so inventory, cards, the gate and the ladder can
...
The sample fits inside one day's budget and, at 108 to 216 targets, inside the
current 256 ceiling before any capacity work lands. The large corpus is
budget bound rather than rate bound: 285.60 USD against a 50 USD weekly cap is
about six weeks. That is the intended shape. A backfill that takes a month is
...
the original files are unavailable, and the 108-file sample projects fewer than
256 embedding targets.
...
| 10    | P2-70j Card embedding targets and the chunk opt-in                  | 2    | A published document adds one or two targets, a chunk is eligible only under the opt-in, and the frozen scorer does not move.                       |
...
ladder without a gate would escalate on nothing. P2-70j depends on the target
table from the first index-capacity PR. P2-70l lands last on purpose: the
literal name is stored with evidence from P2-70c onward, so every entity
question answers before binding exists, and binding is additive.
...
## 15. Open questions and the least certain decisions

| Item                                                     | State                                                                                                                                                                                                                                                                                                      |
| -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The event entity for a generic card                      | Requiring a configured subject entity per source is the safe answer and it blocks generic cards for any source that has not set one. The alternative, a document-scoped entity per source item, would add entity rows with no meaning outside cards. Decide before P2-70c.                                 |
| Absent versus false for clause booleans                  | Rule 7 means "did that instrument have an MFN clause" answers "not recorded" for a document whose text does not mention one. That is correct and it is less useful than "no". A later negation playbook could turn silence into a checked absence; it needs its own evidence rule and is not in this plan. |
| One embedding target per document                        | It is right for the four question shapes above and untested for the fifth. Recall at one target per document is a retrieval-quality question that needs a frozen evaluation on a grown corpus, which this plan cannot answer.                                                                              |
