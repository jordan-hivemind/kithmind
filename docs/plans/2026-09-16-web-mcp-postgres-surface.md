...
| `models/spaces/people` list, create, `setMemberPerson`                                                                                                | family space manager               | P2-39h. `identity/spaces.ts` line 162 says the module lands with entities. i5 ported the rest of the family space manager (spaces, invitations, roles, transfer, leave) and left person-linking rendering a typed "not available on this surface yet" note rather than a half port |
...
This table is the *target*, at row m's flip and after i7b's removal. i7a
lands `requiredMcpEnvironmentVariables`' postgres-surface required set --
`NEXT_PUBLIC_CONVEX_URL`, `MCP_OAUTH_ENCRYPTION_KEY`, `MCP_PUBLIC_ORIGIN`,
`KITH_DATABASE_URL`, `KITH_SESSION_SECRET` -- and `scripts/check-self-hosting.mjs
--web`'s matching preflight, but keeps `NEXT_PUBLIC_CONVEX_URL` required on
both surfaces rather than replacing it, because the 17 MCP tools still reach
Convex on the `postgres` branch of `lib/mcp/reads.ts` and `writes.ts` until
this row's own repointing and i7b's removal are both done; see
`docs/self-hosting.md`'s new PostgreSQL surface section for the complete
variable table by name, reader and required-or-optional.
...
`capture_thought` is the one write with the same problem twice, and its shape is
authorize, call out, re-authorize and apply. One `SERIALIZABLE` transaction
authorizes, resolves the destination with `resolveWriteSpace` and runs every
provider-free branch; it is not read only because `resolveWriteSpace` may
bootstrap the personal space. Then the embedding call. Then one
`REPEATABLE READ READ ONLY` transaction re-authorizes and gathers the similar
memories and covering facts, over the resolved destination alone so nothing from
another space reaches the classifier. Then the classification call. Then one
`SERIALIZABLE` transaction re-authorizes and applies the decision. Three
transactions for a full capture, one for a capture that stops at a provider-free
branch or a denial, which is what the row i4 tests assert. The two
re-authorizations are the point: a key can be revoked while either provider call
is in flight, and the write must not land on the strength of an authorization
taken before it. An unavailable classifier, or an embedding the gate asked for
and did not get, stores nothing and answers `needs_confirmation` with "Memory
was not stored because the admission check was unavailable", which is Convex's
own fail-closed branch.

The gate asks for an embedding only when the destination has one compatible
fingerprint *and* a complete thought index. The second half is not an edge
case: a stored capture leaves an eligible, uncovered target, so I9 reports the
index incomplete and the vector leg would drop every candidate, which means
the capture after any capture would have shipped its text to the provider for
a vector that was then discarded. When the index is not complete the store
takes its keyword candidate leg instead and nothing outbound happens. The
classifier is therefore never asked to judge novelty against an empty
comparison set, and a byte-identical retry by the same author inside
`CAPTURE_RETRY_WINDOW_MS` is refused by the store without consulting either
leg.
...
| Surface                              | File                                                                                                                         | Convex query                                              | Replacement, under `postgres`                                                                                                                                                                                                       |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Worker heartbeat                     | Split in i6: `components/convex-worker-heartbeat-status.tsx` (unchanged) and `components/kith-worker-heartbeat-status.tsx`, both rendered per `fs` source account from `components/kith-settings.tsx` | `diagnostics.public.status`                                 | `useStatusPoll` (`lib/kith/use-status-poll.ts`) polls `GET /api/status/worker?sourceAccountId=` every 10 seconds. The route (`app/api/status/worker/route.ts`) answers `lib/kith/worker-status.ts`'s `loadWorkerStatus`, which resolves the id against the caller's own authorized source accounts (`sources.listSourceAccounts`) and applies `workers.watcherStaleness` -- not `workers.getWorkerDiagnosticsStatus`, whose worker-credential authorization (`requireWorkerSourceAccount`) refuses every web session outright, since a web session principal carries no `credentialId` |
| Dashboard counts and recent thoughts | `components/kith-dashboard.tsx`, initial value handed in by `app/(authenticated)/page.tsx`                                   | `thoughts.public.getStats`, `thoughts.public.listRecent`  | The same `useStatusPoll` hook polls `GET /api/status/dashboard` every 10 seconds. The route (`app/api/status/dashboard/route.ts`) calls `lib/kith/dashboard.ts`'s `computeDashboardData` on the session `withPrincipalRead` opens; `loadDashboard`, the loader the server component calls for the first paint, calls that same function on its own session, so the two can never disagree about a stat and neither transaction nests inside the other                                        |
...
- The dashboard's first paint comes from the server component (`loadDashboard`),
  so its poll is a refresh and never the only source of the value; it does not
  fetch immediately on mount. The worker heartbeat widget has no such
  server-rendered value -- `kith-settings.tsx` is unchanged by i6 and still
  renders it with only a `sourceAccountId` -- so it keeps the Convex version's
  "Loading worker heartbeat..." first state and fetches immediately on mount.
  `use-status-poll.ts`'s `immediate` option is this one difference; everything
  else about the two surfaces' poll is the same code path.
...
| i4    | Write and ingest tools, worker and ingest routes                         | `lib/mcp/writes.ts` new (one method per write and ingest tool, one implementation per surface, `reads.ts`'s shape), `lib/mcp/server.ts` write tools and credential seam (i3's unported-gateway stub deleted), `lib/mcp/auth.ts` (i2's `authenticateApiKeyOnConvex` pin deleted), `app/api/ingest/route.ts`, `app/api/worker/route.ts`, `lib/ingest/http.ts` and `lib/worker/http.ts` (the published code-to-status tables exported, so both surfaces answer one contract from one mapping). `packages/kith-store/src/memory/captureAdmission.ts` new: the three provider-free gate helpers ported out of `@repo/db`'s `memoryAnalysis.ts`, because i7 deletes that dependency and removing the import mechanically would have dropped the content bound and the derived-age refusal with no failing test. `capture_thought` shipped in i4 without the model-backed admission gate, taking its ADD branch unconditionally. The i4 follow-up recorded that and corrected the two published claims it made false. **The gate is ported by the i4 capture-gate follow-up** (`packages/kith-store/src/memory/capture{Admission,Classifier}.ts` and `capture.ts`, `apps/web/src/lib/kith/capture.ts`), which restores fail-closed behaviour, the SKIP rules for sensitive content, duplicate detection, supersession and extracted metadata, and returns `idempotentHint` and the tool description to their Convex values. Duplicate detection does not rest on the vector index alone, because on PostgreSQL it cannot: a stored capture leaves an eligible, uncovered target, I9 then reports the space's thought index incomplete, and the vector leg drops every candidate until a fill runs. So `searchCaptureCandidates` falls back to the keyword leg whenever the index is incomplete, and `applyCaptureDecision` refuses a byte-identical retry by the same author inside `CAPTURE_RETRY_WINDOW_MS`. A scheduled fill is the third leg and is owed: `embedding_fill` has no registered handler, which gates row m. See the module comment in `writes.ts`                                                                                                                                                                        | 2    | `postgres-writes.test.ts`: write denied without `write`, capture denied without read and write, ingest denied without `ingest`, destination resolution over Personal, an explicitly named granted space and a configured default write space, a space the credential was not granted refused in words that name no space id, one `BEGIN ISOLATION LEVEL SERIALIZABLE` per call including per denial, a key revoked between two calls denied on the second, the provider-free gate branches storing nothing, `/api/ingest` idempotent by `requestId`, `/api/worker` identical in status and body to `handlePostgresWorkerRequest` on the worker suite's own fixtures, and the route's error table checked code for code against the store's own `WORKER_ERRORS`. Capture gate: one case per classifier branch with the decision injected, `capture_thought` opening three transactions around its two provider calls and one when it stops before either, a classifier that returns nothing or throws storing nothing and naming no provider, a key revoked during the provider calls denying the write, and the Quick Capture core through the same gate. Over a real seeded index: the vector leg reaching one space only, an incomplete index taking the keyword leg without calling the embedder, a retried identical capture storing one memory, and an embedder that fails or disagrees with the index failing closed. Store side: `captureAdmission.test.mjs` for the provider-free helpers, `memoryCapture.test.mjs` for the candidate and covering-fact reads, both spaces' isolation, every apply branch and the supersede embedding-target bookkeeping. `surface-routing.test.ts` replaces `surface-pinning.test.ts`: each route resolves the bearer on the flagged backend and reaches no other                                                                                                                     | i2, e for both ingest paths                          | Yes  | Yes    |
| i5    | Pages, components, hooks                                                 | `app/(authenticated)/page.tsx`, `browse/page.tsx`, `settings/page.tsx`, `spaces/page.tsx`, `app/invite/page.tsx` split into `components/{convex,kith}-{dashboard,browse,settings,family-space-manager,invitation-acceptance}.tsx`; new `lib/kith/{page-session,dashboard,browse,settings-data,family-data}.ts` (one `withKithReadTransaction` loader per page, session checked inside via `requireWebPrincipal`); new `lib/kith/api-route.ts` plus 11 mutation routes under `app/api/kith/{api-keys,settings,source-accounts,family}/*` (one `withKithTransaction` per call); delete `lib/family-api.ts`. `QuickCapture.tsx` and `ThoughtsView.tsx` keep their Convex behavior and gain no PostgreSQL port (see the report: no ported service classifies raw text into a thought); the PostgreSQL dashboard and browse pages render that gap as a typed unavailable note. `packages/kith-store/src/identity/family.ts` gained `detachMembershipFromInvitations`, a bug fix found while wiring the spaces page: `removeFamilyMember` and `leaveSharedSpace` failed on `family_invitations_membership_id_fkey` for an approved invitation | 1    | `apps/web`: `lib/kith/postgres-pages.test.ts` (one case per loader: two-space isolation, one `BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY` via the `postgres-reads.test.ts` recording-pool pattern, unauthenticated denial), `lib/kith/no-convex-import.test.ts` (no Convex import on the postgres path; each page's redirect is wired to its own loader's `null`, not middleware), `app/api/kith/mutations.test.ts` (create/list/revoke, cross-user isolation on every mutation, the family invite-accept-approve-leave lifecycle). `packages/kith-store`: a case added to `test/identityFamily.test.mjs` for the membership/invitation foreign key fix                                                                                | i1, i3, h for people, question 1 for source accounts | Yes  | No     |
| i6    | Live surfaces and status routes                                          | `components/worker-heartbeat-status.tsx` split into `components/convex-worker-heartbeat-status.tsx` (unchanged) and `components/kith-worker-heartbeat-status.tsx`; `components/kith-dashboard.tsx` (now a client component that polls); `lib/kith/worker-status.ts` new (`loadWorkerStatus`, the read-time predicate against the caller's own authorized source accounts); `lib/kith/dashboard.ts` gains `computeDashboardData`, the computation `loadDashboard` and the new route both call on their own already-open session; `lib/kith/poll.ts` new (the pure poll state machine) and `lib/kith/use-status-poll.ts` new (the hook both live surfaces share, sending `Content-Type: application/json` on every poll so `guardedRequest` does not refuse it); `lib/kith/clock.ts` new (`kithNow`/`setKithNow`, the test-only clock seam `withPrincipalRead` reads); `lib/kith/api-route.ts`'s `withPrincipalRead` (unified with the P2-39i5 review's `withPrincipalRead` rather than a second `withReadPrincipal`) is what both routes below run through, so they also get that review's surface, origin and content-type gate and its `ProofError` mapping; `app/api/status/worker/route.ts` and `app/api/status/dashboard/route.ts` new; `app/(authenticated)/page.tsx` comment only, no behavior change; `components/kith-settings.tsx` wires the new component in place of the "not available on this surface yet" note | 1    | `lib/kith/poll.test.ts` (hidden tab skips, a failed poll keeps the last good value and flags it, a success replaces it -- no jsdom, so this is the whole test of the poll design); `app/api/status/status.test.ts`, database-backed: `stale: false` inside the window and `stale: true` past it from one `setKithNow`-fixed clock, a principal in space A never sees space B's watcher or incident (`400 Source account not found`, indistinguishable from a nonexistent id), unauthenticated is `401` on both routes and `loadDashboard(null)` is `null`, a cross-origin request and a non-JSON content type refused on both routes, a fixed `404` under the `convex` surface on both routes, `not_configured` and a missing `sourceAccountId`, `Cache-Control: no-store` on every response, one `BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY` per request | i5, j for the staleness contract                     | Yes  | No     |
| i7a   | Self-hosting docs and checker for the postgres surface, Quick Capture's route wired onto the i4 follow-up's already-landed admission gate, the embedder-seam and `@repo/db` import cleanups the cutover needs. Convex code stays; the flag stays `convex` in production | `docs/self-hosting.md` (new PostgreSQL surface section, including the classifier's `ANTHROPIC_API_KEY` row), `scripts/check-self-hosting.mjs` (`POSTGRES_WEB_REQUIRED_VARIABLES`, surface-aware `validateWebEnvironment`), `lib/kith/rate-limit.ts` (comment only), `lib/mcp/embedder.ts` new (the one embedder seam `reads.ts`, `writes.ts` (through `capture.ts`'s `runCaptureThought`) and `lib/kith/capture.ts` all resolve through; `capture.ts`'s previously separate `embedder`/`defaultEmbedder` are gone, `setCaptureEmbedder` re-exports this seam), `lib/mcp/reads.ts` (re-exports the seam), `lib/mcp/principal.ts` (`webPrincipalLoader`, `mcpPrincipalLoader`'s web-session sibling: `captureThoughtFromWeb` opens one to three of its own transactions and needs a `WithMcpPrincipal` that reloads the session cookie on each one, which neither `withPrincipal` nor `withPrincipalRead` can give it), `lib/kith/api-route.ts` (`guardedRequest` exported for the capture route to call directly; two new `VALIDATION_MESSAGES`), `app/api/kith/thoughts/capture/route.ts` new (calls the real `captureThoughtFromWeb` the i4 follow-up already shipped), `components/kith-quick-capture.tsx` new, `components/kith-dashboard.tsx` (wires it in), `lib/worker/http.ts`, `app/api/worker/route.ts`, `lib/ingest/http.ts` (repointed off `@repo/db`'s thin re-exports), `apps/web/package.json` (`@repo/worker-protocol` added) | 1 | `scripts/check-self-hosting.test.mjs` (postgres-surface cases), `app/api/kith/thoughts/capture/route.test.ts` (stored via the classifier's ADD decision with the embedder seam asserted uncalled without a ready index, a retried identical capture reported as a duplicate by the gate's own retry guard with no second row, an unavailable classifier failing closed to needs_confirmation, unauthenticated 401, cross-origin 403, convex-surface 404), `lib/kith/no-convex-import.test.ts` (unchanged, covers the two new files by directory scan) | i5, i6, the i4 capture-gate follow-up | Yes | No |
| i7b   | Cutover switch and Convex removal from `apps/web`                        | `apps/web/package.json` (Convex dependencies removed), remaining Convex imports (`lib/mcp/convex-auth.ts` and its test, the JWKS route, every `convex*`/`@repo/db` import i7a did not already repoint, including `lib/mcp/server.ts`'s `parseSpaceReadErrorData` and `lib/ingest/http.ts`'s `parseInlineIngestErrorData`, both of which i7a left in place because neither has a PostgreSQL-side equivalent), `validateMcpEnvironment` required list (`NEXT_PUBLIC_CONVEX_URL` dropped) | 1    | `scripts/check-self-hosting.mjs --web`, no `convex` in the web dependency tree, no `NEXT_PUBLIC_CONVEX_URL` reference                                                                                                                                                                                       | i7a, the production flip at row m                                  | No   | No     |
