...
| `list_review_queue` | readOnly           | read       | `records.mcpQueries.listReviewQueue`   | Missing, see 1.5                                                                 |
...
| `timeline_thoughts` | readOnly           | read       | `thoughts.mcpActions.timeline`         | Missing, see 1.5                                                                 |
| `get_stats`         | readOnly           | read       | `thoughts.mcpQueries.getStats`         | Missing, see 1.5                                                                 |
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Thought space stats digest (`_computeSpaceStats`)                                                                                                     | `get_stats`, dashboard             | Nobody. P2-39g2 landed the counters it reads (`readSpaceCounters` and `spaceEmbeddingCoverage` in `embeddings/state.ts`) but did not port the digest, which had no consumer. Assign to i3 |
...
| Thought timeline                                                                                                                                      | `timeline_thoughts`                | Nobody. Assign to i3                                                                                                                                                                      |
| Record review queue (`models/records/reviewQueue.ts`)                                                                                                 | `list_review_queue`                | Nobody. Assign to i3                                                                                                                                                                      |
| Inline ingest admit, process, result                                                                                                                  | `/api/ingest`                      | P2-39e                                                                                                                                                                                    |
| URL ingest enqueue (`models/ingestion/urlQueue.enqueue`)                                                                                              | `ingest_url`                       | P2-39e                                                                                                                                                                                    |
| Source account create, update, list for the owner UI                                                                                                  | settings page                      | P2-39d scope, merged without it. Needs a new row, question 1                                                                                                                              |
...
`getMcpIssuer()` is still needed. It is the public origin used in
`WWW-Authenticate`, the OAuth metadata documents and the resource identifier,
which is a different fact from the JWT issuer that happened to share the
variable. i2 renames the variable to `MCP_PUBLIC_ORIGIN` and keeps the same
...
production still reads Convex. The flag flips in row m, not here. Three slices
cannot be dark and are marked below.

| Slice | Scope                                                                    | Files                                                                                                                                                                                                                                                        | Tier | Tests it adds                                                                                                                                                                                                                                                                                               | Depends on                                           | Dark | Review |
| ----- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ---- | ------ |
| i1    | Session, login, middleware, pool, read-only transaction, app-role grants | `app/api/auth/*` new, `middleware.ts`, `app/layout.tsx`, `app/(authenticated)/layout.tsx`, delete `ConvexClientProvider.tsx`, `app/sign-in`, `app/sign-up`, `packages/pg/src/transaction.ts`, `packages/kith-store/src/index.ts`                             | 2    | Cookie forge and truncation rejected, logout revokes server side, revoked session denied mid-request, Scrypt fixture from a stored secret verifies, middleware redirect matrix, read-only transaction refuses a write, one grant test per table group                                                       | none beyond c                                        | No   | Yes    |
| i2    | MCP and OAuth authentication, route, environment                         | `lib/mcp/auth.ts`, delete `convex-auth.ts`, `app/api/mcp/route.ts`, `token/route.ts`, `authorize/complete/route.ts`, delete the JWKS route, `lib/mcp/environment.ts`, `lib/mcp/oauth*.ts`, `app/mcp/authorize/page.tsx`, `components/space-grant-picker.tsx` | 2    | Bearer authenticates and a revoked key does not, a `preparing` or `pending` key authenticates nothing, consumed-code replay revokes, exchange identity is refused as an MCP credential, the route binds the authenticated credential and ignores body identity, environment validation names variables only | i1                                                   | Yes  | Yes    |
| i3    | Read tools, plus the timeline and review-queue services                  | `lib/mcp/server.ts` read tools, `lib/mcp/record-query.ts`, `lib/mcp/finance.ts` context, new `memory/timeline.ts` and `records/reviewQueue.ts`                                                                                                               | 2    | One test per tool against the Convex answer for the same synthetic fixture, two-space isolation per tool, `vectorStatus` surfaced when the embedder fails, one transaction per call                                                                                                                         | i2, g2 for `get_stats`                               | Yes  | Yes    |
| i4    | Write and ingest tools, worker and ingest routes                         | `lib/mcp/server.ts` write tools, `app/api/worker/route.ts`, `app/api/ingest/route.ts`                                                                                                                                                                        | 2    | Write denied without `write`, ingest denied without `ingest`, destination space resolution, worker wire parity against `handlePostgresWorkerRequest`, ingest idempotency by `requestId`                                                                                                                     | i2, e for both ingest paths                          | Yes  | Yes    |
...
i1 is not dark because a browser holds one session cookie and the app cannot
authenticate two ways at once for one request. i7 is not dark because removing
the flag is the switch. i6 is dark but pointless before i5, so it lands after.
...
| 4. `get_stats` and the dashboard counts need the thought stats digest, which P2-39g2 did not port although its counters landed. Port the digest in i3, or ship i3 and i5 with `get_stats` returning a typed unavailable result? | Port the digest in i3 over `readSpaceCounters`; it is small now that the counters exist. Until i3 lands, i5 renders the dashboard counts as unavailable, the same shape `list_spaces` already uses for uncounted coverage |
| 5. Rename `MCP_JWT_ISSUER` to `MCP_PUBLIC_ORIGIN`, which is a deployment environment change, or keep the old name to avoid touching the deployment?                                                                             | Rename. The variable outlives the JWT, and keeping the name leaves the next reader looking for a signer that no longer exists                                                                                             |
