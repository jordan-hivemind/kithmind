...
| 2. `authRateLimits` is not migrated and plan 1.2 says "rate limit at the route". Ship i1 with a per-process token bucket, or block i1 on a durable `kith.auth_rate_limits` table?                                               | Ship the token bucket and open a row for the durable table. A serverless per-process bucket is weaker than the Convex table, and saying so is better than delaying the session. One user, pre-launch                      |
