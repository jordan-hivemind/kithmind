...
| i3    | Read tools, plus the timeline and review-queue services                  | `lib/mcp/reads.ts` new (one method per read tool, one implementation per surface), `lib/mcp/server.ts` read tools, new `memory/timeline.ts`, `memory/stats.ts` and `records/reviewQueue.ts`, `embeddings/search.ts` (`recallCandidates` returns thought scores). `lib/mcp/record-query.ts` and the `lib/mcp/finance.ts` context needed no change: the schema is surface independent and the finance context is i2's `authorizedSpaceIds` | 2    | `postgres-reads.test.ts`: one case per read tool on PostgreSQL, a two-space isolation case per tool, `vectorStatus` under a failing injected embedder, one `BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY` per read call and one `SERIALIZABLE` for `query_records`, and a byte-for-byte comparison against hand-written Convex rows for `list_spaces`, `search_facts`, `browse_recent`, `get_thoughts`, `timeline_thoughts` and `get_stats`. Store side: `memoryTimeline.test.mjs`, `memoryStats.test.mjs`, `recordsReviewQueue.test.mjs` | i2, g2 for `get_stats`                               | Yes  | Yes    |
