...
3. **Citations.** Any synthesized output (narrative, brief, summary) must cite `fact:<id>` or `thought:<id>` for every factual claim. No naked claims. Prefer `fact:<id>` when a structured fact records the attribute, since it owns that predicate.
