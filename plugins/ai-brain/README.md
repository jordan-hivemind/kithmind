...
AI Brain is a thin Claude Code plugin over a hosted MCP server. The server stores your thoughts, people, projects, and documents; the plugin exposes skills that let Claude read and write that store as part of your workflow.
...
- **`/weekly-review`** — Weekly synthesis of the week's thoughts against what the brain already holds. Every claim cites its source.
...
- **Cite sources in responses.** When you ask Claude a question grounded in your brain, expect answers with `fact:<id>` or `thought:<id>` citations.
