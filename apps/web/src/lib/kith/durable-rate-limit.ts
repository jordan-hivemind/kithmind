// The authentication rate limit, spent from `kith.auth_rate_limits`.
//
// Section 8 question 2 of the web and MCP surface plan shipped i1 with an
// in-process token bucket and opened a row for `kith.auth_rate_limits`, "where
// the real limit lands". This is that row: the sign-in and sign-up routes spend
// from the durable table (`@repo/kith-store/identity` `consumeAuthAttempt`), so
// a burst spread across a serverless deployment's instances hits one shared
// counter rather than a per-instance budget multiplied by however many
// instances are live. i7b deleted the token bucket with the `convex` surface
// that was the last thing selecting it.
//
...
/** The fallback key for an unattributable request: counted under one shared
 * budget, never exempt. `clientAddress` returning null must not be the cheapest
...
/**
 * The one call the sign-in and sign-up routes make: spends one attempt from
 * `kith.auth_rate_limits` and reports the outcome in a shape the caller does
 * not need to branch on any further.
 */
