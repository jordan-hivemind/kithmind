...
function toSummary(record: SourceAccountListRow): SourceAccountSummary {
  return {
    ...
    freshnessMs: record.freshness_ms === null ? 0 : Number(record.freshness_ms),
...
export async function disconnectSourceAccount(
  ...
  const account = await row<{
    ...
    ctx,
    "SELECT space_id, enabled, disconnected_at FROM kith.source_accounts WHERE id = $1 FOR UPDATE",
    [id],
  );
  if (!account) sourceAccountNotFound();
  try {
  ...
  const affected = await rows<{ id: string }>(
    ctx,
    "UPDATE kith.source_roots SET state = 'retired', updated_at = $1 WHERE source_account_id = $2 AND space_id = $3 AND state <> 'retired' RETURNING id",
    [new Date(ctx.now), id, account.space_id],
  );
  if (account.disconnected_at === null) {
    if (account.enabled) {
      ...
      await onSourceEnabledChanged(
        ctx,
        { id, spaceId: account.space_id, enabled: true },
        false,
      );
    }
    await exec(
      ctx,
      "UPDATE kith.source_accounts SET enabled = false, disconnected_at = $3, disconnected_by = $4 WHERE id = $1 AND space_id = $2",
      [
        id,
        account.space_id,
        new Date(ctx.now),
