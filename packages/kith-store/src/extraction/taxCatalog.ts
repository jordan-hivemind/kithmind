...
const requiredText = (name: string): SeedField => ({
  ...text(name),
  required: true,
});
...
export const FEDERAL_INDIVIDUAL_RETURN: SeedDocumentType = {
  ...
  area: "tax",
  description: "A federal individual income tax return and its front schedules.",
  guidance:
    "Read the front Form 1040 and the attached Schedules 1, 2, 3, A, D, and E that are part of this return. Identify the printed form names and tax year. Capture totals and carry amounts from their printed labels, not assumed modern line positions. Preserve the original printed page evidence. A blank box is absent, not zero. Do not infer a signature, filing status, draft or filed state, amended state, jurisdiction, or any amount that is not printed. Ignore attached K-1s, brokerage packages, and other supporting documents as separate documents so their amounts are not counted again.",
  fields: [
    number("tax_year", true),
    text("filing_status"),
    text("return_version"),
    requiredText("jurisdiction"),
