...
function cacheFor(
  ...
  if (cache && cache.client !== client) {
    throw new Error("Record hydration cache belongs to another database transaction");
  }
  return (
    cache ??
...
async function getRow<T>(
  ...
  if (result.rowCount !== 1) throw new Error(`Stored ${table} identity is not unique`);
...
function requireObservationValue(value: unknown): ObservationValue {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
  ...
  switch (typed.type) {
    case "decimal":
      ...
      break;
    case "money":
      requireExactKeys(value, ["type", "amount", "currency"], "Stored observation value");
      break;
    case "integer":
      requireExactKeys(
        value,
...
function requireDocTypePatch(value: unknown): CardDocTypePatch | null {
  ...
  if (!Array.isArray(value)) throw new Error("Stored document type patch is malformed");
...
function observationFromRow(row: Record<string, unknown>): ObservationRow {
  ...
  const occurrence = requireOccurrence(value.occurrence);
  requireDateOrNull(value.occurrenceInstant, "Stored observation occurrence instant");
  return {
    ...value,
    eventType: requireEventType(value.eventType),
    occurrence,
...
function entityFromRow(row: Record<string, unknown>): EntityRow {
  ...
  if (!["person", "organization", "project", "place", "other"].includes(value.kind)) {
...
function requireEventSchema(
  ...
  if (schemaVersion !== 1) throw new Error("Only record schema version 1 is supported");
  if (isCardRecordKind(type)) {
    ...
    throw new Error("Vehicle service must belong to an other-kind vehicle entity");
  }
}

function requireObservationSchema(row: ObservationRow): void {
  if (!OBSERVATION_TYPE_PATTERN.test(row.observationType)) {
    throw new Error(
  ...
  if (isCardRecordKind(row.eventType)) {
    requireCardObservationSchema(
      row.eventType,
      row.observationType,
      row.observationKey,
    ...
    row.eventType === "financial_transaction" &&
    row.value.type !== "money"
  ) {
    throw new Error("Financial transaction line items must use money values");
  } else if (
    row.eventType === "lab_panel" &&
    !["decimal", "integer", "text", "boolean"].includes(row.value.type)
  ) {
    throw new Error(
...
async function requireGenerationChain(
  ...
  if (revision.kind === "inline_utf8_v1" && text.kind === "inline_text_v1") {
    if (
      ...
      throw new Error("Generation inline source hash or byte length is invalid");
    }
...
export async function hydrateEventVersion(
  ...
  const proof = boundedEvidenceProjection(evidence, eventEvidenceIds(eventVersion));
  return {
    ...chain,
