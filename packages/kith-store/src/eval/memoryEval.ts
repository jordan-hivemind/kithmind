...
// packages/convex/convex/models/thoughts/memoryEval.ts.
//
// Pure scoring over already-recorded results -- no embedding call, no
// language model, no database. That is what makes it usable on both sides of
// the migration: the Convex original scores a recorded Convex retrieval, and
// `recallParity.ts` in this directory scores a recorded PostgreSQL one with
// the exact same function, so a difference in the two reports is a retrieval
// difference and never a scoring difference. Nothing below is changed from
// the Convex source, including the comments; the only edits are this header
// and `export` names, which were already exported there.
