// Ported from packages/convex/convex/models/provenance/representations.ts
// (P2-39d). Pure functions: no database access, so the port is close to
// line-for-line. What changed is only the storage type each function takes --
// a snake_case `kith.source_revisions` / `kith.source_text_versions` row,
// mapped to camelCase by `src/provenance/rows.ts`, in place of a Convex
// `Doc<"sourceRevisions">` / `Doc<"sourceTextVersions">`.
//
// This is the PR178 invariant this row was told to keep: the generation chain
// resolves for both an inline UTF-8 revision (legacy implicit shape, or the
// explicit `inline_utf8_v1` representation) and an archived binary
// representation, and likewise for an inline text version versus one a
// parser produced (`parsed_pages_v1`). `parseSourceRevisionRepresentation`
// and `parseSourceTextRepresentation` are the one place that distinction is
// decided; every other function in this port calls through them rather than
// re-deciding it.
...
export type SourceRevisionShape = {
...
function requireLegacyArchiveRef(value: string | null | undefined): void {
  if (value != null) {
    requireBoundedUtf8(
      value,
...
export function parseSourceRevisionRepresentation(
  ...
  if (value.representation == null) {
    if (
      ...
      value.contentHashAuthority != null
    ) {
...
export function parseSourceTextRepresentation(
  ...
  if (
    (value.evidenceSealed &&
      value.textHashAuthority !== "server_verified_retained_text") ||
    (!value.evidenceSealed && value.textHashAuthority != null)
  ) {
