 ...
 */
const DOCUMENT_PARSER_FAILURE_CODES = new Set<MetadataFirstPreviewGap["code"]>([
  "conversion_failed",
  "conversion_output_invalid",
  "page_limit_exceeded",
  "bundle_too_large",
...
function isDocumentParserFailureCode(
  code: string,
): code is MetadataFirstPreviewGap["code"] {
  return DOCUMENT_PARSER_FAILURE_CODES.has(
    code as MetadataFirstPreviewGap["code"],
  );
}
...
function providerProofFresh(
  ...
  const times =
    provider.referenceVersion === "provider_original_v2"
      ? [verifiedAt]
      : [verifiedAt, provider.locator.readbackVerifiedAt];
  if (times.some((at) => at === undefined)) return false;
...
function processingTerminal(
  ...
  return {
    ...
    ...(checkpoint.metadataFirstDeferred === undefined
      ? {}
      : { metadataFirstCarry: checkpoint.metadataFirstDeferred.metadataFirst }),
  };
}
...
function processingBase(checkpoint: {
  ...
  return {
    ...
    ...(checkpoint.metadataFirstDeferred === undefined
      ? {}
      : { metadataFirstDeferred: checkpoint.metadataFirstDeferred }),
  };
}

function metadataFirstDeferred(
  checkpoint: Extract<RunnerCheckpoint, { phase: "discovery_reserve" }>,
) {
  const routing = checkpoint.metadataFirstCarry;
  ...
  return {
    ...
    pdfIndex: routing.triageStartIndex,
    archivedPublished: checkpoint.archivedPublished ?? 0,
    metadataFirst: routing,
  };
}

function deferredArchivedCheckpoint(
  checkpoint: Extract<
    RunnerCheckpoint,
    {
      phase:
  ...
  const deferred = checkpoint.metadataFirstDeferred;
  if (!deferred) {
  ...
  return {
    ...
    mode: deferred.mode,
    scanId: checkpoint.scanId,
    inventoryEpoch: deferred.inventoryEpoch,
    manifestVersion: deferred.manifestVersion,
    files: deferred.files,
    missingBindings: deferred.missingBindings,
    pdfIndex: deferred.pdfIndex,
    step: "deferred_idle",
    reservationRound: 0,
    archivedPublished: checkpoint.published,
    metadataFirst: { ...deferred.metadataFirst, refreshReady: false },
  };
}
...
export class PipelineRunner {
  ...
  private async nextMetadataCheckpoint(
    ...
    return {
      ...
      ...activeScanBase({
        ...checkpoint,
        metadataFirstCarry: { ...routing, refreshReady: false },
      }),
      round: 0,
      archivedPublished,
    };
  ...
  private async startCycle(
    ...
    const prior =
      ...
      (this.journal.checkpoint.phase === "terminal"
        ...
        : []);
    const metadataFirstCarry = metadataFirstOverride ??
      (this.journal.checkpoint.phase === "terminal"
    ...
    const result = await this.mutation(
      ...
      async (current, response) => {
        ...
        if (value.done === true) {
          ...
          if (pdfIndex >= 0) {
            // Checkpoints written before metadata-first adoption have no
            // marker. Finish that already-open scan with its original deep
            // behavior; only a fresh cycle or the explicit archived-phase
            // adoption command may opt a scan into preview routing.
            if (current.metadataFirstCarry === undefined) {
              return {
                version: 1,
                phase: "archived",
  ...
  private archiveConfiguration(role: ArchiveCopyRole) {
    const pdf = this.requirePdfConfig();
    if (role === "primary") return pdf.archive.primary;
    if (!pdf.archive.independentBackup)
      throw new PipelineWorkerError("archive_backup_configuration_missing");
    return pdf.archive.independentBackup;
  }
  ...
  private providerDeclaration(
    ...
    if (provider.referenceVersion === "provider_original_v2") {
      return {
        ...
        ...base,
        createdAt: provenanceCreatedAt(original.createdAt, verified.verifiedAt),
      };
    }
  ...
  private async adoptLegacyProviderCheckpoint(
    ...
    if (pending) {
      if (
        pending.operation !== "discovery.preflightArchived" ||
        pending.result === undefined
      )
        throw new PipelineWorkerError(
          "provider_v2_transition_requires_answered_pending",
        );
      let body: Record<string, unknown>;
      try {
        body = JSON.parse(pending.requestBody) as Record<string, unknown>;
      } catch {
        throw new PipelineWorkerError("journal_phase_conflict");
      }
      if (
      ...
      const code = errorCode(response);
      if (code) {
        return scanTerminal(checkpoint, "failed", code, true);
      }
      ...
      if (
        ...
        value.archiveIntentDigest !== body.archiveIntentDigest
      )
    ...
    const migratedRows =
      original.providerOriginal?.referenceVersion === "provider_original_v2"
        ? { original, processing }
        : await this.requireCatalog().adoptProviderOriginalV2({
            originalCatalogId: original.originalCatalogId,
            expectedOriginalRevision: original.rowRevision,
            processingCatalogId: processing.processingCatalogId,
            expectedProcessingRevision: processing.rowRevision,
    ...
    return archivedBase(checkpoint, {
      step: "parser_archive",
      preflightAction: undefined,
      expectedOriginalRevision: migratedRows.original.rowRevision,
      expectedProcessingRevision: migratedRows.processing.rowRevision,
    });
  }

  async prepareProviderV2Transition(): Promise<RunnerCheckpoint> {
    const checkpoint = this.journal.checkpoint;
    if (
      checkpoint.phase !== "archived" ||
  ...
  /**
   * A preview refusal caused by the selected bytes is a durable triage gap,
   * not a reason to retry the same file forever. Infrastructure and sandbox
   * failures never enter this path and remain run-fatal.
   */
  private async recordMetadataPreviewGap(
    checkpoint: ArchivedCheckpoint,
    code: MetadataFirstPreviewGap["code"],
  ): Promise<void> {
    if (
      checkpoint.step !== "preview" ||
      checkpoint.metadataFirst === undefined ||
      !isDocumentParserFailureCode(code)
    ) {
      throw new PipelineWorkerError("journal_phase_conflict");
    }
    const identity = metadataIdentity(this.archivedPlan(checkpoint));
    const key = metadataIdentityKey(identity);
    const routing: MetadataFirstRouting = {
      ...checkpoint.metadataFirst,
      previewGaps: checkpoint.metadataFirst.previewGaps.some(
        (gap) => metadataIdentityKey(gap) === key,
      )
        ? checkpoint.metadataFirst.previewGaps
        : [...checkpoint.metadataFirst.previewGaps, { ...identity, code }],
    };
    await this.journal.transitionCheckpoint({
      checkpoint: await this.nextMetadataCheckpoint(
        checkpoint,
        routing,
        checkpoint.archivedPublished,
      ),
  ...
  private async driveArchivedPreflight(): Promise<void> {
    ...
    if (
      checkpoint.preflightAction !== "initial" &&
      checkpoint.preflightAction.includes("backup")
    ) {
      const pdf = this.requirePdfConfig();
      if (!pdf.archive.independentBackup)
        throw new PipelineWorkerError("archive_backup_configuration_missing");
  ...
  private providerCredentials() {
    ...
    if (
      provider.remoteName !== undefined &&
      provider.rcloneBinary !== undefined &&
      provider.configPath !== undefined &&
      provider.configIdentityFingerprint !== undefined
    ) {
      return {
        rcloneBinary: provider.rcloneBinary,
        configPath: provider.configPath,
        remoteName: provider.remoteName,
        configIdentityFingerprint: provider.configIdentityFingerprint,
      };
    }
  ...
  private async verifyProviderOriginal(
    original: OriginalCatalogRow,
    plan: FilePlan,
    capturePath: string,
  ) {
    const account = this.requirePdfConfig().providerOriginal;
    if (!account || !original.providerOriginal)
      throw new PipelineWorkerError("provider_original_configuration_missing");
    const provider = providerBinding(account, plan.rootAlias);
    const loaded = await verifyDropboxOriginal({
      ...
      relativePath: plan.relativePath,
      bindingId: original.providerOriginal.bindingId,
      ...(original.providerOriginal.verified === undefined
        ? {}
        : {
            expectedProviderFileIdHash:
              original.providerOriginal.verified.providerFileIdHash,
          }),
    });
  ...
  private async driveProviderOriginal(
    checkpoint: ArchivedCheckpoint,
    original: OriginalCatalogRow,
    _processing: ProcessingCatalogRow,
    capturePath: string,
    ...
    if (authorizedAction === "provider_verify") {
      ...
      const next = providerV2.verified
        ...
        : await this.requireCatalog().recordProviderVerified({
            catalogId: original.originalCatalogId,
            expectedRevision: original.rowRevision,
            verified,
          });
    ...
    if (!state.verified || !providerProofFresh(state)) {
      await this.journal.transitionCheckpoint({
        checkpoint: archivedBase(checkpoint, {
          step: "preflight",
    ...
    const result = await this.mutation(
      ...
      async (current, response, pending) => {
        ...
        const originalReceipts = [
          ...(providerV2
            ? []
            : [["primary", "originalPrimaryReceiptId"] as const]),
          ...(provider
            ? []
    ...
    const result = await this.mutation(
      ...
      async (current, response, pending) => {
        ...
        if (value.found !== true) {
          ...
          const reuse = parserArtifactReuse(value);
          const providerV2 =
            mapped.original.providerOriginal?.referenceVersion ===
            "provider_original_v2";
          if (
            reuse !== undefined &&
  ...
  private admissionSelections(
    ...
    const providerV2 =
      mapped.original.providerOriginal?.referenceVersion ===
      "provider_original_v2";
    const originalSelections: ArchiveReceiptSelection[] = [];
    if (original?.primaryReceiptId !== undefined) {
      ...
      if (original.backupReceiptId !== undefined)
        originalSelections.push(
          existingArchiveReceiptSelection({
            subjectKind: "original_bytes",
    ...
    const parserSelections: ArchiveReceiptSelection[] = [
      reuse
        ? existingArchiveReceiptSelection({
    ...
    if (reuse?.backupReceiptId !== undefined)
      parserSelections.push(
        existingArchiveReceiptSelection({
          subjectKind: "parser_output",
          copyRole: "independent_backup",
    ...
    else if (!reuse && mapped.processing.copies.independent_backup)
      parserSelections.push(
        createArchiveReceiptSelection(
          "parser_output",
          mapped.processing,
    ...
    return {
      ...
      archives: [...originalSelections, ...parserSelections],
    };
  }
    ...
    if (reuse?.providerReferenceId !== undefined) {
      return {
        existingProviderOriginal: {
          ...
          ...(reuse.providerReferenceVersion === undefined
            ? {}
            : { referenceVersion: reuse.providerReferenceVersion }),
        },
      };
    }
    ...
    const result = await this.mutation(
      ...
      (current, response) => {
        ...
        if (value.state === "complete") {
          return current.metadataFirstDeferred
            ? deferredArchivedCheckpoint(current)
            : processingTerminal(current, "complete", undefined, assessmentId);
        }
        return processingTerminal(
          current,
          "incomplete",
      ...
      case "archived":
        if (checkpoint.step === "deferred_idle") {
          ...
          return {
            ...
            ...(checkpoint.metadataFirst.previewGaps.length === 0
              ? {}
              : {
                  previewGaps: checkpoint.metadataFirst.previewGaps.length,
                  previewGapCodes: [
                    ...new Set(
                      checkpoint.metadataFirst.previewGaps.map(
                        (gap) => gap.code,
                      ),
                    ),
                  ].sort(),
    ...
    for (let steps = 0; steps < 10_000; steps += 1) {
      ...
      try {
        ...
        if (
          ...
          isDocumentParserFailureCode(error.code) &&
          checkpoint.phase === "archived" &&
          checkpoint.step === "preview" &&
          !this.journal.pending
        ) {
          await this.recordMetadataPreviewGap(checkpoint, error.code);
          continue;
        }
        if (
          error instanceof ParserProcessError &&
